...
// 【底层优化】定期清理 pendingMap 防止内存泄漏（保留最近2分钟的pending记录）
setInterval(() => {
  const now = Date.now();
  for (const [key, { cancel, timestamp }] of pendingMap.entries()) {
  ...
  config.cancelToken =
    ...
    new axios.CancelToken((cancel) => {
      if (!pendingMap.has(pendingKey)) {
        // 【底层优化】存储cancel函数+时间戳，支持定期清理
        pendingMap.set(pendingKey, { cancel, timestamp: Date.now() });
      }
    });
}
